...
public class BOJ2470_BinarySearch {
    ...
    public static void solution() throws IOException {
        ...
        for (int i = 0; i < N - 1; i++) {
            ...
            int optimalPairValue = findOptimalPair(arr, i + 1, N - 1, arr[i]);
            // i번째 값(value)과 value에 대한 최적값인 optimalPairValue 더해서 확인해줘야함
            int sumAbs = Math.abs(arr[i] + optimalPairValue);

            // 두 용액의 합이 0에 가장 가까운 쌍을 저장
            if (answerAbs > sumAbs) {
                answerAbs = sumAbs;
                answer1 = arr[i];
                answer2 = optimalPairValue;
            }
        }
    ...
    public static int findOptimalPair(int[] arr, int fromIndex, int toIndex, int value) {
        ...
        int optimalPairAbs = Math.abs(value + optimalPairValue);
        ...
        while (left <= right) {
            ...
            // 기존에 지정한 절댓값보다 이번 단계의 절댓값이 더 작으면 pairValue 바꿔줌
            if (sumAbs < optimalPairAbs) {
                optimalPairValue = arr[mid]; // 실제값
                optimalPairAbs = sumAbs; // 절대값
            }

            // sum이 음수면 + 값을 더해줘야 절대값이 작아짐, 그러므로 다음에 조사해야 할 값이 arr[mid] 값보다 더 큰 값 (정렬되어 있기 때문에)
            if (sum < 0) left = mid + 1;
            // sum이 양수이면 - 값을 더해줘야 절대값이 작아질 가능성이 있음, 그러므로 왼쪽 확인해야 함 (오른쪽 값 바꿔주기)
